class HoH_MultiColumnList extends MapVoteMultiColumnList;
...
function LoadList(VotingReplicationInfo LoadVRI, int GameTypeIndex)
{
	...
	if( Divide(A,"|",A,B) )		Split(B, ",", SkipList);
	Split(A, ",", PrefixList);
		...
		for( p=0; p<PreFixList.Length; p++)
		{
			if( left(MP, len(PrefixList[p])) ~= PrefixList[p] )
			{
				for( l=(SkipList.Length-1); l>=0; --l )
					if( left(MP, len(SkipList[l])) ~= SkipList[l] ) break;
				if( l!=-1 )					continue;
...
function ApplyFilter(string filter)
{
	...
	if (len(filter) < 2 && OldFilter == "")		return; // require at least 2 letter to start filtering

	filter 		= caps(filter);
	if (filter == OldFilter)					return;

	// Since the developers were too retarded to use ItemCount from native code, we cannot simply call Clear(), we need to call AddedItem/UpdatedItem/RemovedItem for each entry instead
...
function DrawItem(Canvas Canvas, int i, float X, float Y, float W, float H, bool bSelected, bool bPending)
{
	...
	if ( bSelected)
	{
		...
		MapName 	= "- > " $ MapName;
		MState 		= MSAT_Focused;
		CellLeft   += 8; // Menor desplazamiento si está seleccionado
	} else {
		DrawStyle 	= Style;
		MState 		= MenuState;
		CellLeft   += 12; // Más desplazado si NO está seleccionado
	}
	...
	DrawStyle.DrawText(Canvas, MState, CellLeft, Y, CellWidth, H, TXTA_Left,
		HoH_VotingReplicationInfo(VRI).RepArray[MapVoteData[SortData[i].SortItem]], MyFontScale);
}
function string GetSortString( int i )
{
	local string ColumnData[5];

	if (i >= MapVoteData.Length)		return "";
	...
	ColumnData[3] = HoH_VotingReplicationInfo(VRI).SortedArray[MapVoteData[i]];
...
defaultproperties
{
    ...
    GetItemHeight=HoH_MultiColumnList.MyItemHeight
    FontScale=1
}