class HoH_MapListLoader extends DefaultMapListLoader;
...
function LoadFromMapList(string MapListType, xVotingHandler VotingHandler)
{
	...
	for(i=0;i<Maps.Length;i++)
	{
		...
		// Parse map string incase there are mutator and game options in it DOM-Aztec?Game=XGame.xDoubleDom?mutator=XGame.MutVampire,UTSecure.MutUTSecure?WeaponStay=True?Translocator=True?TimeLimit=15
		Parts.Length = 0;
		...
		if (Right(MapName, extlen) ~= DefaultMapExt)	MapName = Left(MapName, Len(MapName) - extlen); // remove file extension
...
function LoadFromPreFix(string Prefix, xVotingHandler VotingHandler)
{
	...
	for (i = 0; i < LoadedPrefixes.Length; ++i) {
		if (LoadedPrefixes[i] ~= Prefix)			return;
	}
