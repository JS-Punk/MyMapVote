// ====================================================================
//  HoH_VotingHandler - Modification by Marco
// ====================================================================
class HoH_VotingHandler extends xVotingHandler
	Config(HoH_MapVote);
...
{
	MapListLoaderType = string(Class'HoH_MapListLoader');
	Super.LoadMapList();
}
...
function SubmitMapVote(int MapIndex, int GameIndex, Actor Voter)
{
    ...
    if( GameIndex>=GameConfig.Length || (MapIndex!=-999 && (MapIndex<0 || MapIndex>=MapList.Length)) )
        return; // Something is wrong...

    // Solo verificamos IsValidVote para mapas no aleatorios
    if(MapIndex != -999 && !IsValidVote(MapIndex, GameIndex)) 
        return;
    ...
    if (!bSpectatorsCanVote && PRI.bOnlySpectator && Level.Game.NumPlayers > 0)      return;

    if(MapIndex != -999 && (!MapList[MapIndex].bEnabled || (MVRI[Index].MapVote==MapIndex && MVRI[Index].GameVote==GameIndex)))
        return;

    if(MapIndex == -999)
        log("___" $ Index $ " - " $ PRI.PlayerName $ " voted for Random Map (" $ GameConfig[GameIndex].Acronym $ ")",'MapVote');
    else
    ...
    if(bAccumulationMode)
    {
        if(bScoreMode)
        {
            ...
            if(MapIndex == -999)
                TextMessage = repl(TextMessage, "%mapname%", "Random Map" $ "(" $ GameConfig[GameIndex].Acronym $ ")" );
            else
                TextMessage = repl(TextMessage, "%mapname%", MapList[MapIndex].MapName $ "(" $ GameConfig[GameIndex].Acronym $ ")" );
            Level.Game.Broadcast(self,TextMessage);
        }
...
{
    local int i, WinningMapVotes, WinningIndex, WinningGame;
    local bool bTie;
    local MapHistoryInfo MapInfo;
    ...
    if(bLevelSwitchPending || MapVoteCount.Length == 0) 
        return;
    ...
    {
        if(MapVoteCount[i].VoteCount > WinningMapVotes)
        {
            WinningMapVotes = MapVoteCount[i].VoteCount;
            WinningIndex = MapVoteCount[i].MapIndex;
            WinningGame = MapVoteCount[i].GameConfigIndex;
            bTie = false;
        }
        else if(MapVoteCount[i].VoteCount == WinningMapVotes)
            bTie = true;
    }

    // Si no hay votos o hay empate y no es forzado, retornamos
    if(WinningMapVotes <= 0 || (bTie && !bForceMapSwitch))
        return;

    // Si el ganador es Random Map, ahora elegimos el mapa real
    if(WinningIndex == -999)
    {
        WinningIndex = Rand(MapCount);
        log("Random Map selected: " $ MapList[WinningIndex].MapName,'MapVote');
    }

    CloseAllVoteWindows();
    bLevelSwitchPending = true;
    MapInfo = History.PlayMap(MapList[WinningIndex].MapName);
    ServerTravelString = SetupGameMap(MapList[WinningIndex], WinningGame, MapInfo);
...
function AddMapVoteReplicationInfo(PlayerController Player)
{
	...
	M = Spawn(class'HoH_VotingReplicationInfo',Player,,Player.Location);
	if(M == None)
	{
...
function Timer()
{
	...
	if(ScoreBoardTime > -1)
	{
		if(ScoreBoardTime == 0)			OpenAllVoteWindows();
		ScoreBoardTime--;
		return;
	}
	TimeLeft--;
...
function AddMap(string MapName, string Mutators, string GameOptions) // called from the MapListLoader
{
	...
	Class'HoH_MapRepHistory'.Static.GetMapHistoryRep(MapName,RepArray[MapCount].Positive,RepArray[MapCount].Negative);
	...
	if(bUpdate)		History.AddMap(MapInfo);
}
// Using this function to save map reputation aswell.
function CloseAllVoteWindows()
{
	local int i,Pos,Neg;
	local HoH_VotingReplicationInfo R;

	for(i=0; i < MVRI.Length;i++)
	{
		R = HoH_VotingReplicationInfo(MVRI[i]);
		if( R!=none )
		{
			switch( R.MapRepVote )
			{
			...
			case 2:
				...
				break;
			default:
				log("MapVote CloseAllVoteWindows MapRepVote=" $ R.MapRepVote);
			}
			R.CloseWindow();
		}
	}
	log("MapVote CloseAllVoteWindows Pos=" $ Pos $ " Neg=" $ Neg);
	if( Pos!=0 || Neg!=0 )
...
// Modificamos GetDefaultMap para manejar el caso de Random Map
function GetDefaultMap(out int mapidx, out int gameidx)
{
	...
	while( bLoop )
	{
		...
		if( MapList[i].bEnabled )
		{
			...
			{
				if( left(MapList[i].MapName, Len(PL[x])) ~= PL[x] )	break;
			}
			if( x>=0 )
			{
				for( x=(SPL.Length-1); x>=0; --x )
				{
					if( left(MapList[i].MapName, len(SPL[x])) ~= SPL[x] )	break;
				}
				if( x==-1 )	bLoop = false;
			}
		}
		if(bLoop && r++ > 100)
		{
			// give up after 100 unsuccessful attempts. find the first map that matches up to default gametype
...
function string GetConfigArrayData(string ConfigArrayName, int RowIndex, int ColumnIndex)
{
	...
	{
		if( RowIndex > GameConfig.Length-1 || ColumnIndex > 5 ) return "";
