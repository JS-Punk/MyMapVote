Class HoH_LevelCleanup extends Interaction;
...
{
	local int i;
	local HoH_LevelCleanup C;

	for( i=(PC.Player.LocalInteractions.Length-1); i>=0; --i )
		if( PC.Player.LocalInteractions[i].Class==Default.Class )			return;
	C 									= new(None) Class'HoH_LevelCleanup';
	...
	C.Initialize();
	GUIController(PC.Player.GUIController).MapVotingMenu = string(Class'HoH_MapVotingPageX');
}